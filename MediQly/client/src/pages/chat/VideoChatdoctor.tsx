...
const Chat = () => {
  ...
  const { data: availableUsers } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get("/users/available"); // Fetch only available users
      return data;
    },
    enabled: !!currentId, // Ensures query runs only if currentId is defined
  });
      ...
      <div className="container mx-auto animate-in">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold">
            {isInstantConsultation ? 'Instant Consultation' : 'Chat with Patient'}
          </h1>
          {selectedRecipient && (
